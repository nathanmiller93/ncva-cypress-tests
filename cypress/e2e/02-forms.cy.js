const { MinimizeOutlined } = require("@mui/icons-material");
...
describe("Job Submission checks", () => {
  ...
  it("Enters valid info in each field", () => {
    ...
    cy.get('[name="wpforms[fields][1]"]').type("Automation Test " + readableDate); //Your name
    cy.get('[name="wpforms[fields][2]"]').type(timestamp.toISOString() + "@email.com"); //Your email
    ...
    cy.get('[name="wpforms[fields][9]"]').type("£25,000"); //Salary
    ...
    //cy.get('[name="wpforms[fields][14]"]').type(); //How to Apply - Finish
    //cy.get('[name="wpforms[fields][15]"]').type(); //More information - Finish
  });
  });
